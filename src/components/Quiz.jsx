...
const Quiz = () => {
    ...
    const [seletedAnswer, setSelectedAnswer] = React.useState("");
                            ...
                            <li style={{
                                borderColor:
                                    showCorrectAnswer && option === questions[currentQuestion].correctAnswer
                                        ? "lightgreen"
                                        : showCorrectAnswer && seletedAnswer !== option
                                            ? "lightcoral"
                                            : "",

                                cursor: showCorrectAnswer ? "default" : "pointer",
                            }} onClick={() => handleAnswer(option)} className=" has-checked:bg-gray-300 active:bg-gray-200 max-w-[90%] list-none border-2 cursor-pointer border border-gray-400 hover:bg-gray-200 rounded p-2 px-4" key={index} >{option}</li>
                        ))) : (

                            <input style={{ borderColor: showCorrectAnswer && seletedAnswer !== questions[currentQuestion].correctAnswer ? "red" : "green" }} type="number" className='p-2 border border-gray-300 rounded active:outline-none' placeholder='Enter your answer in number ' onChange={(e) => setIntegerAnswers(e.target.value)} value={integerAnswers} />
